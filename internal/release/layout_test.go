...
func TestPublishedLayoutIsProductScopedUnderTools(t *testing.T) {
	if got := PublishedPath("lynx", "stable", "0.9.0"); got != "Tools/lynx/stable/0.9.0" {
		t.Fatalf("unexpected LYNX path: %q", got)
	}
	if got := PublishedPath("usbtoolbox", "beta", "1.2.0-rc.1"); got != "Tools/usbtoolbox/beta/1.2.0-rc.1" {
...
func TestManagedLayoutAndLegacyRedirect(t *testing.T) {
	for _, candidate := range []string{
		...
		"Tools/lynx/stable",
		`/Tools\\usbtoolbox\\beta\\1.2.0`,
	} {
		if !IsManagedPublishedPath(candidate) {
			t.Fatalf("managed path not recognized: %q", candidate)
		}
	}
	for _, candidate := range []string{"Tools", "Tools/lynx", "Tools/lynx/manual", "Tools/lynx/docs/1.0.0", "downloads/releases.zip"} {
		if IsManagedPublishedPath(candidate) {
			t.Fatalf("manual path was reserved: %q", candidate)
		}
	}
	if got, ok := LegacyRedirectPath("/releases/lynx/stable/0.9.0/file.exe"); !ok || got != "/Tools/lynx/stable/0.9.0/file.exe" {
