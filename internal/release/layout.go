...
func legacyPublishedPath(product, channel, version string) string {
	return filepath.ToSlash(filepath.Join(ReleaseDirectoryName, product, channel, version))
}
...
func IsManagedPublishedPath(relative string) bool {
	...
	if normalized == ReleaseDirectoryName || strings.HasPrefix(normalized, ReleaseDirectoryName+"/") {
		return true
	}
...
func LegacyRedirectPath(requestPath string) (string, bool) {
	...
	if len(parts) < 2 || parts[0] != ReleaseDirectoryName || !identifierPattern.MatchString(parts[1]) {
		return "", false
	}
	destination := []string{PublicToolsDirectory, parts[1], ReleaseDirectoryName}
